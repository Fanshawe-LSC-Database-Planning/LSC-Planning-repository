...
<script>
	...
	function validateMandateFormParameters() {
		if (document.getElementById('firstName').value == ''
				...
				|| document.getElementById('emailId').value == ''
				|| document.getElementById('empType').value == '') {
			alert('Please fill atleast the mandatory parameters!');
		} else {
				document.getElementById("addFacultyForm").submit();
	...
	function clearForm() {
<%if (request.getAttribute("message") != null && request.getAttribute("message").toString().contains("success")) {%>
	document.addFacultyForm.reset();
<%}%>
	}
	function openNextWindow(){
		location.href = "http://localhost:8080/nft-tracker/addTerm";
	}
</script>
</head>
<body style="margin-top: 50px;" onload="clearForm()">
	<%
	...
	<header class="w3-display-container w3-content w3-wide"
		...
		<div class="testbox">
			<form:form method="POST" action="createProfessorInSystem"
				modelAttribute="professor" name="addFacultyForm" id="addFacultyForm"
				...
				<fieldset>
					...
					<div class="item">
						<label>Employee Id: </label> <input id="employeeId"
							name="employeeId" type="text" style="width: 100%; padding: 10px" />
					</div>
					...
					<br>
					<form:radiobutton path="empType" id="empType" checked="checked"
						name="empType" value="Partial Load" label="Partial Load" />
				...
				<div class="btn-block">
					...
					<br>
					<%
						if (request.getAttribute("message") != null && request.getAttribute("message").toString().contains("[uk")) {
					%>
					<p style="color: red;" id="message">Record Already Exists with
						the entered email Id!</p>
